...
## Output location (v2.0)

`/review` skill aggregate findings, present user, apply accepted fixes, **append entry vào target's frontmatter `changelog:` list** (KHÔNG body table). Format:

```yaml
changelog:
  - 2026-05-12 | /review | reviewed by @senior-ba, @qa-reviewer: 1 blocking applied, 2 warnings ack
```
